package com.nodewebzsz.ociserver.service.nginx.impl;

import com.nodewebzsz.dao.entity.NginxConfig;
import com.nodewebzsz.dao.entity.ProxyConfig;
import com.nodewebzsz.dao.entity.SslCertificate;
import com.nodewebzsz.dao.entity.SystemConfig;
import com.nodewebzsz.dao.repository.NginxConfigRepository;
import com.nodewebzsz.dao.repository.SslCertificateRepository;
import com.nodewebzsz.dao.repository.SystemConfigRepository;
import com.nodewebzsz.ocicommon.param.ApiResponse;
import com.nodewebzsz.ociserver.service.nginx.NginxConfigService;
import com.nodewebzsz.ociserver.service.nginx.ProxyConfigService;
...
public class NginxConfigServiceImpl implements NginxConfigService {
    ...
    public Map<String, Object> buildInstallCommand(String mode, String publicBaseUrl) {
         ...
         * 3) --network host 部署的 oci-pool 应改用 local 模式（127.0.0.1）
         */
        ...
        // 容器侧环境变量示例（重启 oci-pool 容器时注入，比改 yml 更稳）
        ...
        result.put("hint", "local".equals(m)
                ? "在与 oci-pool 同一台服务器上执行（需 root）。若 oci-pool 使用 --network host，也选本模式。"
        ...
        result.put("steps", "docker".equals(m)
                ...
                "给 oci-pool 容器注入 OPENRESTY_API_BASE_URL 与 OPENRESTY_API_TOKEN 并重启",
                "回到本页点「刷新状态」，应显示已就绪",
