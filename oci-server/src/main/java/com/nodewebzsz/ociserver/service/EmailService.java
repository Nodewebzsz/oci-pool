package com.nodewebzsz.ociserver.service;

import com.nodewebzsz.dao.entity.EmailBody;
import com.nodewebzsz.dao.entity.EmailReceive;
import com.nodewebzsz.dao.entity.EmailSendRecord;
import com.nodewebzsz.dao.entity.Tenant;
import com.nodewebzsz.dao.entity.TenantEmailConfig;
import com.nodewebzsz.ociserver.pojo.request.EmailBodyRequest;
import com.nodewebzsz.ociserver.pojo.request.EmailReceiveAddRequest;
import com.nodewebzsz.ociserver.pojo.request.EmailReceiveRequest;
import com.nodewebzsz.ociserver.pojo.request.EmailSendRecordRequest;
import com.nodewebzsz.ociserver.pojo.request.EmailSendRequest;
import com.nodewebzsz.ociserver.pojo.request.TenantEmailConfigRequest;
import com.nodewebzsz.ocicommon.param.ApiResponse;
import org.springframework.data.domain.Page;

import java.util.List;

public interface EmailService {


    /**
     * 分页查询收件人列表
     */
    Page<EmailReceive> listPage(EmailReceiveRequest request);

    /**
     * 添加收件人
     */
    EmailReceive addReceive(EmailReceiveAddRequest request);

    /**
     * 删除收件人
     */
    void deleteReceive(Long id);

    /**
     * 根据ID查询收件人
     */
    EmailReceive getById(Long id);


    //启用邮箱的租户列表
    Page<TenantEmailConfig> listTenantEmailConfig(TenantEmailConfigRequest request);

    /**
    * @Description: 一键启用邮箱服务(需要先配置cf)
    * @Param: [com.nodewebzsz.dao.entity.Tenant, java.lang.String, long]
    * @return: com.nodewebzsz.ociserver.pojo.response.base.ApiResponse
    * @Author: nodewebzsz
    * @Date: 9/27/25 6:33 PM
    */
    ApiResponse enableEmailForTenant(Tenant tenant, String emailDomainName, long tenantId);

    /**
    * @Description: 邮件发送
    * @Param: [com.nodewebzsz.ociserver.pojo.request.EmailSendRequest]
    * @return: void
    * @Author: nodewebzsz
    * @Date: 9/28/25 10:30 AM
    */
    void send(EmailSendRequest request);

    /**
    * @Description: emailBodyList
    * @Param: [com.nodewebzsz.ociserver.pojo.request.EmailBodyRequest]
    * @return: org.springframework.data.domain.Page<com.nodewebzsz.dao.entity.EmailBody>
    * @Author: nodewebzsz
    * @Date: 9/28/25 12:40 PM
    */
    Page<EmailBody> emailBodyList(EmailBodyRequest request);

    /**
    * @Description: emailSendList
    * @Param: [com.nodewebzsz.ociserver.pojo.request.EmailSendRecordRequest]
    * @return: org.springframework.data.domain.Page<com.nodewebzsz.dao.entity.EmailSendRecord>
    * @Author: nodewebzsz
    * @Date: 9/28/25 12:40 PM
    */
    Page<EmailSendRecord> emailSendList(EmailSendRecordRequest request);

    void deleteEmailBody();
    void deleteEmailBody(EmailBodyRequest request);

    /**
    * @Description: 禁用邮件服务
    * @Param: [com.nodewebzsz.ociserver.pojo.request.TenantEmailConfigRequest]
    * @return: void
    * @Author: nodewebzsz
    * @Date: 9/28/25 4:42 PM
    */
    void disableEmailForTenant(TenantEmailConfigRequest request);

    void deleteEmailConfig(Long tenantId);

    List<TenantEmailConfig> getTenantEmailConfig(String tenantId);

    //根据名称查询
    TenantEmailConfig getTenantEmailConfigByName(String domainName);

    void update(Tenant tenant, TenantEmailConfig tenantEmailConfig);

}
