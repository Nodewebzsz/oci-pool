...
import com.nodewebzsz.ociai.chat.ChatAiConfigService;
import com.nodewebzsz.ociai.utils.OciAiChatUtils;
import com.nodewebzsz.ocicommon.enums.OperatorEnum;
import com.nodewebzsz.ocicommon.enums.RegionEnum;
import com.nodewebzsz.ocicommon.param.ChatAiConfigDto;
import com.nodewebzsz.ociserver.config.telegram.TelegramBotService;
import com.nodewebzsz.ociserver.config.telegram.TelegramUserService;
...
import static com.nodewebzsz.ocicommon.utils.DateTimeUtils.getReadableZoneTime;
    ...
    public ResponseEntity<?> getAiTenants() {
        try {
            ...
            for (Tenant tenant : tenants) {
                ...
                // 下拉展示用：租户名（自定义名优先，其次租户名）+ 区域；name 保持原样（userName/OCID）以兼容既有调用
                String tenantDisplayName = (tenant.getDefName() != null && !tenant.getDefName().isEmpty())
                        ? tenant.getDefName() : tenant.getTenancyName();
                t.put("tname", tenantDisplayName + " - " + RegionEnum.getRegionCode(tenant.getRegion()));
                result.add(t);
            }
            return ResponseEntity.ok(result);
        } catch (Exception e) {
