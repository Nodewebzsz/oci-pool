...
import com.nodewebzsz.ociserver.pojo.request.UsernameRequest;
import com.nodewebzsz.ociserver.pojo.request.VerificationRequest;
...
import static com.nodewebzsz.ocicommon.param.ApiResponse.SUCCESS;
...
public class ApiController  extends BaseController{
    ...
    public ApiResponse getCurrentIp(HttpServletRequest currentRequest) {
        // 取“客户端”真实公网 IP：优先读反代头(X-Forwarded-For/X-Real-IP)，无代理时回退 remoteAddr。
        // 若拿到的是内网/回环地址(如 Docker 网关 192.168.x.x / 172.x.x.x)，说明拿不到真实公网 IP，
        // 返回错误让前端走 ipwho.is / ipify 等第三方服务取浏览器出口 IP(该 IP 会随客户端切换而变化)。
        String clientIp = IpUtils.getClientIpAddress(currentRequest);
        if (StringUtils.isBlank(clientIp) || "unknown".equalsIgnoreCase(clientIp) || PingUtil.isPrivateIP(clientIp)) {
            return ApiResponse.error("client ip is not public");
        }
        String geo = PingUtil.getGeoInfoByIP(clientIp);
        if ("内网地址".equals(geo)) geo = "";
        return ApiResponse.success(SUCCESS, clientIp + "/" + geo);
    }
