package com.nodewebzsz.ociserver.config.datamigration;

import com.nodewebzsz.ocicommon.utils.AesFileEncryptor;
...
import static com.nodewebzsz.ocicommon.utils.ZipUtils.gzip;
...
public class DatabaseExportService {
    ...
    /**
     * 导出“业务表”到内存流（只导出 com.nodewebzsz.dao.entity 下带 @Entity 的表）
     */
    public ByteArrayOutputStream exportDatabaseToStream() throws Exception {
        ...
        try (Connection conn = jdbcTemplate.getDataSource().getConnection()) {
            ...
            writer.println("-- OCI-POOL PLAIN BACKUP (NO ENCRYPTION)");
            writer.println("-- GENERATED AT: " + new Timestamp(System.currentTimeMillis()));
            writer.println();

            Set<String> businessTableNames = getBusinessTableNames("com.nodewebzsz.dao.entity");
    ...
    public void exportEncryptedBackup(HttpServletResponse response) throws Exception {
        ...
        // 4. 封装成可识别的迁移加密文件格式
        StringBuilder sb = new StringBuilder();
        sb.append("-----BEGIN OCI-POOL MIGRATION-----\n");
        sb.append("IV:").append(ivBase64).append("\n");
        sb.append("DATA:").append(cipherBase64).append("\n");
        sb.append("-----END OCI-POOL MIGRATION-----\n");

        // 5. 设置下载响应头
        response.setContentType("application/octet-stream");
        response.setHeader("Content-Disposition", "attachment; filename=\"oci-pool_migration.enc\"");

        // 6. 返回 masterKey 给前端（只显示一次）并暴露响应头
        response.setHeader("X-MASTER-KEY", masterKey);
        response.setHeader("Access-Control-Expose-Headers", "X-MASTER-KEY, Content-Disposition");
