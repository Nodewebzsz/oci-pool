...
import com.nodewebzsz.dao.repository.AppVersionRepository;
...
public class VersionCheckTask {
    ...
    private void executeDockerUpdate() {
        ...
        try {
            ...
            String command = "curl -L $(curl -s " + GITHUB_API_URL + " | " +
                    "grep \"browser_download_url.*jar\" | cut -d '\"' -f 4) -o oci-pool.jar && " +
                    "pkill -f \"oci-pool.jar\" && " +
                    "nohup java -jar oci-pool.jar > /dev/null 2>&1 &";
    ...
    private void executeSshUpdateScript(File scriptFile, File workDir) throws IOException {
        ...
        command.add("-c");
        command.add("nohup setsid " + scriptFile.getAbsolutePath() + " " + updateCommand + " > /tmp/ociPool_update.log 2>&1 &");
