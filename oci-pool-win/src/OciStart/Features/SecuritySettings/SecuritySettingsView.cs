...
public sealed class SecuritySettingsView : UserControl
{
    ...
    private readonly TextBox _mfaIssuer = FormFieldFactory.TextField("OCI-Pool Verify");
    ...
    private async Task SaveMfaAsync()
    {
        try
        {
            var raw = await _api.PostJsonAsync("/api/system/updateMfaConfig", new
            {
                ...
                issuer = string.IsNullOrWhiteSpace(_mfaIssuer.Text) ? "OCI-Pool Verify" : _mfaIssuer.Text.Trim()
            }).ConfigureAwait(true);
