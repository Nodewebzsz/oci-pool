...
final class HeaderViewModel: ObservableObject {
    ...
    private func fetchLatestMacDMGRelease() async throws -> MacDMGRelease? {
        ...
        req.setValue("oci-pool-mac", forHTTPHeaderField: "User-Agent")
    ...
    private static func pickDMGAsset(_ assets: [[String: Any]], tag: String) -> (url: URL, name: String)? {
        ...
        for a in assets {
            ...
            if lower.hasPrefix("ocipool-") && lower.contains(ver) {
                if versioned == nil { versioned = (u, name) }
                continue
            }
            if lower.contains("mac") || lower.contains("ocipool") || lower.contains("oci-start") {
