...
final class BackendController: ObservableObject {
    ...
    func start() async {
        ...
        // Use ~/.ocipool (no spaces). "Application Support" breaks H2/JDBC and
        ...
        for i in 0..<90 {
            ...
            if !proc.isRunning {
                ...
                state = .failed("后端进程退出 (code=\(code))，见 Application Support/OciPool/backend.log")
    ...
    /// Prefer `~/.ocipool/server.jar` (optional override), then bundled `server.jar`.
    ...
    private func killOrphanBackendsOnPort(_ port: Int) {
        ...
        for pid in pids {
            ...
            let isOurs = cmd.contains("server.jar")
                || (cmd.contains("java") && cmd.contains("oci-pool"))
