...
/// Full-window container for the React Modern UI (the same SPA served by the backend).
///
/// 方案 A：整个窗口内容由 WKWebView 加载当前前端，登录页与登录后的业务页直接复用 Web。
/// 客户端只保留：本机 / 远程引导、后端启动、窗口生命周期与 Cookie 同步。
final class ModernWebViewController: NSViewController, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    ...
    override func loadView() {
        ...
        let bar = showsToolbar ? makeToolbar() : nil
        ...
        config.userContentController.addUserScript(
            WKUserScript(source: routeScript, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        )
        ...
        let topAnchor = bar?.bottomAnchor ?? root.topAnchor
    ...
    private func makeToolbar() -> NSView {
        ...
        bar.layer?.backgroundColor = (dark
            ? NSColor(srgbRed: 0x1e / 255, green: 0x21 / 255, blue: 0x24 / 255, alpha: 0.92)
            : NSColor(srgbRed: 0xe4 / 255, green: 0xea / 255, blue: 0xf2 / 255, alpha: 0.95)).cgColor

        let border = CALayer()
        border.backgroundColor = (dark
            ? NSColor(srgbRed: 0x38 / 255, green: 0x3c / 255, blue: 0x40 / 255, alpha: 1)
            : NSColor(srgbRed: 0xb8 / 255, green: 0xc8 / 255, blue: 0xd8 / 255, alpha: 1)).cgColor
        ...
        func iconButton(_ systemName: String, action: Selector) -> NSButton {
            ...
            b.isBordered = false
            b.image = NSImage(systemSymbolName: systemName, accessibilityDescription: nil)
        ...
        let title = NSTextField(labelWithString: "OCI-POOL")
        ...
        let open = NSButton(title: "浏览器打开", target: self, action: #selector(openInBrowser))
        open.bezelStyle = .inline
        open.isBordered = true
        open.font = .systemFont(ofSize: 11, weight: .medium)
        open.contentTintColor = NSColor(srgbRed: 0x1a / 255, green: 0xbc / 255, blue: 0x9c / 255, alpha: 1)
    ...
    private func observeWebView() {
        ...
        titleObs = webView.observe(\.title, options: [.new]) { [weak self] wv, _ in
            DispatchQueue.main.async {
                ...
                if !t.isEmpty { self.titleLabel?.stringValue = t }
            }
        }
    }
        ...
        Task {
            ...
            await MainActor.run {
                ...
                if panel.runModal() == .OK, let dest = panel.url {
                    ...
                    catch { AppAlert.error(message: error.localizedDescription) }
                }
            }
        }
    }
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationResponse: WKNavigationResponse, decisionHandler: @escaping (WKNavigationResponsePolicy) -> Void) {
        ...
        if isAttachment || isDownloadMime, !navigationResponse.canShowMIMEType {
            ...
            saveDownload(from: url, suggestedName: Self.filename(fromContentDisposition: cd))
            return
        }
        if isAttachment {
            decisionHandler(.cancel)
    ...
    func webView(_ webView: WKWebView, runJavaScriptAlertPanelWithMessage message: String, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping () -> Void) {
        AppAlert.info(title: "OCI-POOL", message: message)
        completionHandler()
    }

    func webView(_ webView: WKWebView, runJavaScriptConfirmPanelWithMessage message: String, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping (Bool) -> Void) {
        let ok = AppAlert.confirm(title: "OCI-POOL", message: message, confirmTitle: "确定", cancelTitle: "取消")
        completionHandler(ok)
    }

    func webView(_ webView: WKWebView, runJavaScriptTextInputPanelWithPrompt prompt: String, defaultText: String?, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping (String?) -> Void) {
        let alert = NSAlert()
        alert.messageText = "OCI-POOL"
