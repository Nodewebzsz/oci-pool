...
APP_NAME="OciPool"
JAR_FINAL_NAME="oci-pool-release"
...
cd "$REPO_ROOT"
mvn ${MVN_SETTINGS_ARGS[@]+"${MVN_SETTINGS_ARGS[@]}"} clean install -DskipTests -q --also-make -pl oci-server
...
# 带版本号的 DMG，并保留 OciPool.dmg 副本方便习惯路径
...
#!/bin/bash
set -euo pipefail
APP="/Applications/OciPool.app"
if [ ! -d "$APP" ]; then
  APP="$(cd "$(dirname "$0")" && pwd)/OciPool.app"
fi
if [ ! -d "$APP" ]; then
  osascript -e 'display dialog "请先把 OciPool.app 拖到应用程序，再双击本脚本。" buttons {"好"} default button 1 with title "OCI-POOL"'
  exit 1
fi
xattr -cr "$APP" 2>/dev/null || true
chmod +x "$APP/Contents/MacOS/OciPool" 2>/dev/null || true
open "$APP"
HELPER
    chmod +x "$OPEN_HELPER_DST"
fi
...
正确步骤：
  1. 打开 DMG
  2. 把 OciPool.app 拖到「应用程序」(Applications)
  3. 双击「首次打开.command」
     （会自动清除隔离标记并启动；首次可能提示「无法验证开发者」
      → 系统设置 → 隐私与安全性 → 仍要打开，或终端见下方）

若双击 App 仍提示「无法打开」，在「终端」执行一行即可：
  xattr -cr /Applications/OciPool.app && open /Applications/OciPool.app
...
echo "   1. 把 OciPool.app 拖到「应用程序」"
echo "   2. 双击 DMG 里的「首次打开.command」（清隔离并启动）"
echo "   或终端一行: xattr -cr /Applications/OciPool.app && open /Applications/OciPool.app"
echo ""
echo "💡 App 数据目录（更新不丢失）:"
echo "   ~/Library/Application Support/OciPool/"
echo ""
echo "📦 环境变量:"
