...
public interface TenantRepository extends JpaRepository<Tenant, Long> {
    ...
    */
    ...
    /**
     * 检查父租户的显示/自定义名称是否已被当前云类型占用。
     * 导入接口使用 tenancy_name 承载弹窗中的“自定义名称”，因此必须在服务端
     * 以数据库权威数据校验，不能依赖前端分页列表。
     */
    @Query("SELECT CASE WHEN COUNT(t) > 0 THEN true ELSE false END FROM Tenant t " +
            "WHERE LOWER(t.tenancyName) = LOWER(:tenancyName) AND t.cloudType = :cloudType " +
            "AND (t.parenId IS NULL OR t.parenId = 0) " +
            "AND (:excludeTenantId IS NULL OR t.id <> :excludeTenantId)")
    boolean existsParentByTenancyNameAndCloudType(@Param("tenancyName") String tenancyName,
                                                   @Param("cloudType") Integer cloudType,
                                                   @Param("excludeTenantId") Long excludeTenantId);
