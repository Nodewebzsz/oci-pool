...
import com.nodewebzsz.ocicommon.enums.RecordStatus;
import com.nodewebzsz.ocicommon.enums.RecordType;
...
public interface DnsRecordRepository extends JpaRepository<DnsRecord, Long>, JpaSpecificationExecutor<DnsRecord> {
     ...
     * 查找指定服务商中指定IP的所有A记录
     */
    @Query("SELECT d FROM DnsRecord d WHERE d.providerType = :providerType AND d.recordValue = :ipAddress AND d.recordType = com.nodewebzsz.ocicommon.enums.RecordType.A")
     ...
     */
    @Query("SELECT d FROM DnsRecord d WHERE d.providerType = :providerType AND d.recordValue = :ipAddress AND d.recordType = com.nodewebzsz.ocicommon.enums.RecordType.AAAA")
