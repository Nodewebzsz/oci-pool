...
# 使用示例
# 1. 默认安装（将安装到 /root/oci-pool-docker 目录）：
#    ./docker.sh install
#
# 2. 指定目录安装（三种方式）：
#    a. 一次性指定：
#       OCI_APP_DIR=/root/oci-pool-docker ./docker.sh install
#
#    b. 临时指定（仅对当前终端有效）：
#       export OCI_APP_DIR=/root/oci-pool-docker
#       ./docker.sh install
#
#    c. 永久指定（对当前用户永久有效）：
#       echo 'export OCI_APP_DIR=/root/oci-pool-docker' >> ~/.bashrc
#       source ~/.bashrc
#       ./docker.sh install
#
# 3. 卸载应用：
...
# 配置变量
APP_DIR="${OCI_APP_DIR:-/root/oci-pool-docker}"
APP_TZ="${TZ:-Asia/Shanghai}"
APP_CONTAINER_NAME="oci-pool"
SCRIPT_PATH=$(realpath "$0")
SYMLINK_PATH="/usr/local/bin/oci-pool-docker"
...
create_symlink() {
    if [ ! -L "$SYMLINK_PATH" ] || [ "$(readlink "$SYMLINK_PATH")" != "$SCRIPT_PATH" ]; then
        ...
        if ln -sf "$SCRIPT_PATH" "$SYMLINK_PATH" 2>/dev/null; then
            log_success "软链接创建成功，现在可以使用 'oci-pool-docker' 命令"
        else
...
deploy_app() {
    ...
    # 检查并拉取最新镜像
    log_info "拉取最新镜像..."
    if ! docker pull lovele/oci-pool:latest; then
        log_error "拉取镜像失败"
        return 1
    fi
    ...
    if docker run -d \
        ...
        -p ${PORT}:9856 \
        -v "$APP_DIR/data:/oci-pool/data" \
        -v "$APP_DIR/logs:/oci-pool/logs" \
        -v "$APP_DIR/docker.sh:/oci-pool/docker.sh" \
        -v /var/run/docker.sock:/var/run/docker.sock \
        -v /usr/bin/docker:/usr/bin/docker \
        -e SERVER_PORT=9856 \
        -e OCI_APP_DIR=/oci-pool \
        -e DATA_PATH=/oci-pool/data \
        -e LOG_HOME=/oci-pool/logs \
        -e TZ="$APP_TZ" \
        --restart always \
        lovele/oci-pool:latest; then
...
uninstall() {
    ...
    if [ -d "${app_dir}" ]; then
        ...
        if [ -d "${app_dir}/data" ]; then
            log_info "保护数据目录..."
            local temp_data_dir="/tmp/oci-pool-data-backup"
    ...
    # 清理Docker镜像
    log_info "清理Docker镜像..."
    if docker images | grep -q "lovele/oci-pool"; then
        docker rmi "$(docker images lovele/oci-pool -q)" >/dev/null 2>&1
        if [ $? -eq 0 ]; then
...
update() {
    ...
    # 获取新镜像ID
    local new_image_id=$(docker images lovele/oci-pool:latest --format='{{.ID}}' | head -n1)
...
main() {
    ...
    case "$1" in
        "install")
            ...
            if [ $? -eq 0 ]; then
                log_success "全部部署完成！"
                echo -e "${GREEN}现在可以使用 'oci-pool-docker' 命令来管理应用${NC}"
            else
