...
# 应用配置 - 使用绝对路径
JAR_PATH="/root/oci-pool/oci-pool-release.jar"
LOG_FILE="/dev/null"
JAR_DIR="$(dirname "$JAR_PATH")"
SCRIPT_PATH=$(readlink -f "$0")
SYMLINK_PATH="/usr/local/bin/oci-pool"
...
create_symlink() {
    if [ ! -L "$SYMLINK_PATH" ] || [ "$(readlink "$SYMLINK_PATH")" != "$SCRIPT_PATH" ]; then
        ...
        if ln -sf "$SCRIPT_PATH" "$SYMLINK_PATH" 2>/dev/null; then
            log_success "软链接创建成功，现在可以使用 'oci-pool' 命令"
        else
...
start() {
    ...
    # 输出成功提示
    log_success "环境准备完成，现在可以使用 'oci-pool' 命令"
